...
main() {
  ...
  # And the configuration in English, into the vault, where the phone can read it
  # — refreshed here because the arms it describes have just run, and a page about
  # what is running is worth nothing if it is a week stale. It goes into a
  # SUBFOLDER of the vault: the corpus globs are non-recursive, so it can never
  # become material the curator stitches posts out of (tests/check_brief.sh).
  "$SCRIPT_DIR/brief.sh" --write >/dev/null 2>&1 || log "WARN brief refresh failed"
}
